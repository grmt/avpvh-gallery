...
export class PhotoTagger {
  ...
  private annotationMap: Map<string, TagData> = new Map();
  ...
  private async loadMembers() {
    try {
      const response = await fetch(`/wp-json/avpvh/v1/members/for-tagging`);
  ...
  private attachToPhotoSwipe(container: HTMLElement) {
    ...
    const photoSwipeInitObserver = new MutationObserver(() => {
      const photoLinks = container.querySelectorAll('[data-avpvh-id][data-pswp-width]');
    ...
    photoSwipeInitObserver.observe(container, { subtree: true, childList: true });
  }
  ...
  private async onSlideChange() {
    ...
    if (currentSlide) {
      const imageLink = currentSlide.closest('[data-avpvh-id]') as HTMLElement;
      if (imageLink) {
  ...
  private displayTags(tags: TagData[]) {
    // Render tags as visual overlays on the image
    const pswpImg = document.querySelector('.pswp__img') as HTMLElement;
    if (!pswpImg) return;

    // Clear previous tags
    pswpImg.querySelectorAll('.avpvh-tag-overlay').forEach((el) => el.remove());
    ...
    try {
      const response = await fetch('/wp-admin/admin-ajax.php', {
        ...
        body: new URLSearchParams({
          ...
          _ajax_nonce: (window as any).avpvhShortcodeLocalize?.tag_nonce || '',
        }).toString(),
      });
